...
gui.win = figure('Position', pos, 'Menubar', 'none', ...
    'NumberTitle','off', ...
    'Name', sprintf('DOTTER v. %s ⸺ BiCroLabs 2015-2021', df_version()), ...
    'MenuBar', 'None', ...
    'Color', [1,1,1], ...
    'Resize', 'Off', ...
    'Tag', 'DOTTER', ...
...
uimenu(mDNA, 'Label', 'Export 2D masks', 'Callback', @run_exportMasks, ...
    ...
    function convert_nd2(varargin)       
        ...
        warndlg('DOTTER does not transfer the metadata to the tif files. Please use radiantkit instead.', 'modal');
    end
