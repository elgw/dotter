...
fig_menu_delete()
    ...
    function init_fig_menu()
        ...
        ctrl.slice = uicontrol('Style', 'text', ...
            'String', 'Z-plane ?/?', 'Position', [10,160,150,40], ...
            'HorizontalAlignment','left',...
            'Parent', gui.viewPanel);
        ctrl.sliceJmp = uicontrol('Style', 'pushbutton', ...
            'String', '#', 'Position', [230,160,40,40], ...
            'Callback', @gui_jmpSlice,...
            'Parent', gui.viewPanel);
        ctrl.sliceNext = uicontrol('Style', 'pushbutton', ...
            'String', '>', 'Position', [270,160,40,40], ...
            'Callback', @gui_nextSlice,...
            'Parent', gui.viewPanel);
        ctrl.sumProjection = uicontrol('Style', 'checkbox', ...
            'String', 'SUM', 'value', s.sumProjection, 'Position', [330,160,100,40], ...
            'Callback', @gui_changeProjection,...
            'Parent', gui.viewPanel);
        ctrl.maxProjection = uicontrol('Style', 'checkbox', ...
            'String', 'MAX', 'value', s.maxProjection, 'Position', [385,160,100,40], ...
            'Callback', @gui_changeProjection,...
            'Parent', gui.viewPanel);
    ...
    function gui_jmpSlice(varargin)
         % Jump to any slice
                
        nSlices = size(C{1}, 3);
        sliceStrings = {nSlices, 1};
        for kk = 1:nSlices
            sliceStrings{kk} = sprintf('%d', kk);
        end
        
        [sel,~] = listdlg('PromptString','Select a slice:',...
            'SelectionMode','single',...
            'ListString',sliceStrings);
